...
#define HEAP_CAPACITY_INC 4096
    ...
    static std::vector<tTJSVariantString *> *TJSStringHeapList = nullptr;
    static tTJSVariantString **TJSStringHeapFreeCellList = nullptr;
    ...
    //---------------------------------------------------------------------------
    static void TJSUninitStringHeap() {
        ...
        TJSStringHeapList = nullptr;

        if(TJSStringHeapFreeCellList) {
            delete[] TJSStringHeapFreeCellList;
            TJSStringHeapFreeCellList = nullptr;
        }
    }
//---------------------------------------------------------------------------
...
#define TJS_CHECK_FREE_BLOCK_MAX 50
    ...
    tTJSVariantString *TJSAllocVariantString(const tjs_char *ref1,
        ...
        if(len1 + len2 > TJS_VS_SHORT_LEN) {
            ...
            if(ref1)
                TJS_strcpy(ret->LongString, ref1);
            if(ref2)
                TJS_strcpy(ret->LongString + len1, ref2);
        } else {
            if(ref1)
                TJS_strcpy(ret->ShortString, ref1);
            if(ref2)
