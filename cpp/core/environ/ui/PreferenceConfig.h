...
static std::string PreferenceGetValueString(const std::string &name,
                                            const std::string &defval) {
    return GetConfigManager()->GetValue<std::string>(name, defval);
}
static void PreferenceSetValueString(const std::string &name,
                                     const std::string &v) {
...
namespace {
        ...
        iPreferenceItem *createItem(int idx) override {
            ...
            return CreatePreferenceItem<tPreferenceItemCheckBox>(
                ...
                [this](tPreferenceItemCheckBox *item) {
                    item->_getter =
                        std::bind(&PreferenceGetValueBool, Key, DefaultValue);
                    item->_setter = std::bind(&PreferenceSetValueBool, Key,
                                              std::placeholders::_1);
                });
        }
    };

    class tTVPPreferenceInfoSelectList : public tTVPPreferenceInfo<std::string>,
                                         tPreferenceItemSelectListInfo {
    public:
        tTVPPreferenceInfoSelectList(
            const std::string &cap, const std::string &key,
            const std::string &defval,
        ...
        iPreferenceItem *createItem(int idx) override {
            ...
            return CreatePreferenceItem<tPreferenceItemSelectList>(
                ...
                [this](tPreferenceItemSelectList *item) {
                    item->initInfo(this);
                    item->_getter =
                        std::bind(&PreferenceGetValueString, Key, DefaultValue);
    ...
    class tTVPPreferenceInfoTextureCompressionSelectList
        : public tTVPPreferenceInfoSelectList {
    public:
        tTVPPreferenceInfoTextureCompressionSelectList(
            const std::string &cap, const std::string &key,
            const std::string &defval,
            const std::initializer_list<
                std::tuple<std::string, std::string, unsigned int>> &listinfo) :
            tTVPPreferenceInfoSelectList(
                cap, key, defval,
        ...
        std::vector<std::tuple<std::string, std::string, unsigned int>>
            TextFmtList;
    };

    // class tTVPPreferenceInfoSelectRenderer : public
    // tTVPPreferenceInfoSelectList { 	typedef
    // tTVPPreferenceInfoSelectList inherit; public:
    // 	tTVPPreferenceInfoSelectRenderer(const std::string &cap, const
    // std::string &key, const std::string &defval, 		const
    // std::initializer_list<std::pair<std::string, std::string> >
    // &listinfo) : inherit(cap, key, defval, listinfo) {} 	virtual
    // void onSetValue(const std::string &v) {
    // inherit::onSetValue(v); 		if (v == "opengl") {
    // TVPOnOpenGLRendererSelected(true);
    // 		}
    // 	}
    // };

    class tTVPPreferenceInfoSelectFile
        : public tTVPPreferenceInfo<std::string> {
    public:
        tTVPPreferenceInfoSelectFile(const std::string &cap,
        ...
        iPreferenceItem *createItem(int idx) override {
            ...
            return CreatePreferenceItem<tPreferenceItemFileSelect>(
                ...
                [this](tPreferenceItemFileSelect *item) {
                    item->_getter =
                        std::bind(&PreferenceGetValueString, Key, DefaultValue);
                    item->_setter = std::bind(&PreferenceSetValueString, Key,
                                              std::placeholders::_1);
                });
        }
    };
    ...
    public:
        tTVPPreferenceInfoRendererSubPref(const std::string &cap) {
            Caption = cap;
        } // Key is useless
    ...
    public:
        ...
        iPreferenceItem *createItem(int idx) override {
            ...
            ret->addClickEventListener([this](Ref *) {
                TVPMainScene::GetInstance()->pushUIForm(
                    TVPGlobalPreferenceForm::create(&Preference));
            });
            return ret;
        }
    ...
    public:
        ...
        iPreferenceItem *createItem(int idx) override {
            ...
            tPreferenceItemCursorSlider *ret = new tPreferenceItemCursorSlider(
                DefaultValue, TVPMainScene::convertCursorScale);
            ret->autorelease();
            ret->_getter =
                std::bind(&PreferenceGetValueFloat, Key, DefaultValue);
            ret->_setter =
                std::bind(&PreferenceSetValueFloat, Key, std::placeholders::_1);
    ...
    public:
        tTVPPreferenceInfoSliderText(const std::string &cap,
                                     const std::string &key, float defval) :
            tTVPPreferenceInfo<float>(cap, key, defval) {}
    ...
    public:
        ...
        iPreferenceItem *createItem(int idx) override {
            ...
            ret->addClickEventListener([](Ref *) {
                LocaleConfigManager *locmgr =
                    LocaleConfigManager::GetInstance();
                if(TVPShowSimpleMessageBoxYesNo(
                       locmgr->GetText("preference_ensure_reset_skin"),
    ...
    public:
        ...
        iPreferenceItem *createItem(int idx) override {
            ...
            ret->addClickEventListener([this](Ref *) {
                TVPMainScene::GetInstance()->pushUIForm(
                    KeyMapPreferenceForm::create(GetConfigManager()));
            });
            return ret;
        }
    };
    ...
    static void initAllConfig() {
        ...
        RootPreference.Preferences = {
            new tTVPPreferenceInfoCheckBox("preference_output_log", "outputlog",
                                           true),
            new tTVPPreferenceInfoCheckBox("preference_show_fps", "showfps",
                                           true),
            new tTVPPreferenceInfoSelectList("preference_fps_limit",
                                             "fps_limit", "60",
                                             { { "60", "60" },
                                               { "45", "45" },
                                               { "30", "30" },
            ...
            new tTVPPreferenceInfoSelectFile("preference_default_font",
                                             "default_font", ""),
            new tTVPPreferenceInfoCheckBox("preference_force_def_font",
            ...
            new tTVPPreferenceInfoSliderIcon("preference_virtual_cursor_scale",
                                             "vcursor_scale", 0.5f),
            new tTVPPreferenceInfoSliderText("preference_menu_handler_opacity",
                                             "menu_handler_opa", 0.15f),
            new tTVPPreferenceInfoKeyMap("preference_keymap"),
#ifdef GLOBAL_PREFERENCE
            new tTVPPreferenceInfoCheckBox("preference_remember_last_path",
        ...
        SoftRendererOptPreference.Preferences = {
            new tTVPPreferenceInfoSelectList(
                "preference_multi_draw_thread", "software_draw_thread", "0",
            ...
            new tTVPPreferenceInfoSelectList(
                "preference_software_compress_tex", "software_compress_tex",
                "none",
        ...
        OpenglOptPreference.Preferences = {
            new tTVPPreferenceInfoSubPref(
                "preference_opengl_extension_opt",
                {
                    new tTVPPreferenceInfoConstant(
                        "preference_opengl_extension_desc"),
#ifdef CC_TARGET_OS_IPHONE
                    new tTVPPreferenceInfoCheckBox(
                    ...
                    new tTVPPreferenceInfoCheckBox(
                        ...
                        "GL_ARM_shader_framebuffer_fetch", true),
                    new tTVPPreferenceInfoCheckBox(
                        "GL_NV_shader_framebuffer_fetch",
                        "GL_NV_shader_framebuffer_fetch", true),
                    new tTVPPreferenceInfoCheckBox("GL_EXT_copy_image",
                                                   "GL_EXT_copy_image", false),
                    new tTVPPreferenceInfoCheckBox("GL_OES_copy_image",
                                                   "GL_OES_copy_image", false),
                    new tTVPPreferenceInfoCheckBox("GL_ARB_copy_image",
                                                   "GL_ARB_copy_image", false),
                    new tTVPPreferenceInfoCheckBox("GL_NV_copy_image",
                                                   "GL_NV_copy_image", false),
                    new tTVPPreferenceInfoCheckBox(
                        "GL_EXT_clear_texture", "GL_EXT_clear_texture", true),
                    new tTVPPreferenceInfoCheckBox(
                        "GL_ARB_clear_texture", "GL_ARB_clear_texture", true),
                    new tTVPPreferenceInfoCheckBox("GL_QCOM_alpha_test",
                                                   "GL_QCOM_alpha_test", true),
                }),
            new tTVPPreferenceInfoCheckBox("preference_ogl_accurate_render",
                                           "ogl_accurate_render", false),
            //		new
            // tTVPPreferenceInfoCheckBox("preference_opengl_dup_target",
            //"ogl_dup_target", true),
            new tTVPPreferenceInfoSelectList(
                "preference_ogl_max_texsize", "ogl_max_texsize", "0",
            ...
            new tTVPPreferenceInfoTextureCompressionSelectList(
                "preference_ogl_compress_tex", "ogl_compress_tex", "none",
                {
                    std::make_tuple("preference_ogl_compress_tex_none", "none",
                                    0),
                    std::make_tuple("preference_ogl_compress_tex_half", "half",
                                    0),
                    std::make_tuple("ETC2", "etc2",
            ...
            // 		new
            // tTVPPreferenceInfoSelectList("preference_ogl_render_tex_quality",
            // "ogl_render_tex_quality", "1", { 			{
