...
tjs_error tTVPSimpleOptionProvider::GetValue(
    ...
    try {
        if(!dest)
            return TJS_E_FAIL;
        tjs_error er = Object.PropGet(0, name, nullptr, dest, nullptr);
        if(TJS_FAILED(er))
            return er;
        return TJS_S_OK;
    } catch(...) {
...
{
	if(bpp != 8 && bpp != 32) return nullptr; // invalid bitmap color depth
...
tjs_error tTVPCrossFadeTransHandlerProvider::StartTransition(
    ...
    /* destination layer type */ /*in*/ tjs_uint src1w, tjs_uint src1h,
    /* source 1 size */ /*in*/ tjs_uint src2w, tjs_uint src2h,
    /* source 2 size */ /*out*/ tTVPTransType *type,
    /* transition type */ /*out*/ tTVPTransUpdateType *updatetype,
    /* update typwe */ /*out*/
    iTVPBaseTransHandler **handler /* transition handler */) {
    if(type)
